...
public class SignatureCreator {
	...
	public PrivateKey getPrivate() throws IOException {
		LogMessage.debug(log, "getPrivate started:" + PRIVATE_KEY_PATH);
		...
		LogMessage.log(log, "Read file successfully!!! PRIVATE_KEY_PATH:" + PRIVATE_KEY_PATH);

		if (key.contains("-----BEGIN PRIVATE KEY-----")) {
			LogMessage.debug(log, "inside IF BEGIN PRIVATE KEY");
			...
			try {
				...
				LogMessage.debug(log, "IN IF About to generate & return privateKey");
			...
			LogMessage.debug(log, "inside ELSE");

			Security.addProvider(new BouncyCastleProvider());
			try {
				KeyFactory factory = KeyFactory.getInstance("RSA");

				ClassPathResource resource2 = new ClassPathResource(PRIVATE_KEY_PATH);
				InputStreamReader reader = new InputStreamReader(resource2.getInputStream(), StandardCharsets.UTF_8);
				...
				LogMessage.debug(log, "Read File PRIVATE_KEY_PATH:" + PRIVATE_KEY_PATH);


				PKCS8EncodedKeySpec privKeySpec = new PKCS8EncodedKeySpec(content);
				pemReader.close();

				LogMessage.debug(log, "IN ELSE About to generate & return privateKey");
